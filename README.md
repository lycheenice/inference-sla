...
- **PD 模式合并** — `mergePdSnapshots(p, d)` 把 prefill 与 decode 两个 snapshot 合并为单一 `[PD-DISAGG]` 视图：用户视角指标（TTFT / TPOT / E2E / genThroughput / outputTokenRate / kvUsage / spec）取 decode 端，prefill 侧输入与缓存（inputTokenRate / L1 / L2 / cached_* / L1 容量与占用 / evicted / load_back 迁移计数与速率）求和（两端各自独立），per-stage 序列拼接两端所有阶段，KV TRANSFER 直方图取有样本的一端（通常 prefill 发送方），pdQueues 按角色分别取值，kvPool 与并发/HTTP 求和。totalRequests / abortedRequests 取两端 max 以避免双计。
